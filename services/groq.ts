...
export const callGroqApiWithTools = async (messages: any[], tools: any[], model: string = "llama-3.3-70b-versatile") => {
    // Tool support is limited in unified AI gateway for now. Falling back to standard call.
...
export const callGroqApiStream = async (messages: any[], onChunk: (text: string) => void, model: string = "llama3-8b-8192") => {
    // Streaming is simulated in unified gateway for now (Full response at once)
    const text = await callGroqApi(messages, model);
    onChunk(text);
    return text;
};
