...
export interface AIModel {
  id: string; // e.g., 'groq-llama3'
  provider: 'Groq' | 'Gemini' | 'Claude' | 'OpenAI' | 'DeepSeek' | 'Mistral' | 'Together' | 'Fireworks' | 'Cohere' | 'Perplexity' | 'OpenRouter' | 'HuggingFace' | 'Ollama' | 'vLLM' | 'Qwen' | 'Yi' | 'Baichuan' | 'Zhipu' | 'Meta LLaMA';
  apiKeys: string[];
  currentKeyIndex: number;
  enabled: boolean;
  priority: number; // 1 (Highest) to 99
...
export interface AIFeatureMap {
  [feature: string]: string[]; // e.g., 'notes': ['groq-llama3', 'gemini-pro']
}
